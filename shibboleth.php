<?php
/*
 Plugin Name: Shibboleth 2
 Plugin URI: https://github.com/ck-ws/shibboleth
 ...
 Author: Will Norris, mitcho (Michael 芳貴 Erlewine) - maintained by Christoph Kreutzer
 Version: 2.0
 License: Apache 2 (http://www.apache.org/licenses/LICENSE-2.0.html)
 GitHub Plugin URI: https://github.com/ck-ws/shibboleth
...
/**
 * Handles various formats of environment variables
 * from sources getenv() and $_SERVER global
 */
function shibboleth_getenv($var) {
    ...
    foreach ($check_vars as $check_var => $true) {
        ...
        elseif ( isset($_SERVER[$check_var]) ) {
...
function shibboleth_activate_plugin() {
	...
	$roles = array(
		'administrator' => array(
			'header' => 'entitlement',
			'value' => array('urn:mace:example.edu:entitlement:wordpress:admin'),
		),
		'author' => array(
			'header' => 'affiliation',
			'value' => array('faculty'),
...
function shibboleth_get_user_role() {
	...
	foreach ( $wp_roles->role_names as $key => $name ) {
		...
		if ( count( array_filter(array_intersect($role_value, $values)) ) > 0 ) {
			$user_role = $key;
			break;
		}
	}
